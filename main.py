import discord
from discord import ui, app_commands
from discord.ext import commands
import os
import random

# --- הגדרות בסיסיות (מושך מה-Variables של Railway) ---
TOKEN = os.getenv('DISCORD_TOKEN')
MY_USER_ID = 1130542850883469443

# IDs של הרולים (מעודכן לפי מה שביקשת)
ROLE_SUPPORTER = 1503819239310627068
ROLE_VIP = 1503817695466881255
ROLE_TICKET_STAFF = 1501316672345211041

# מערכת יתרות (בזיכרון)
user_balances = {}

# --- עיצוב הכפתורים של החנות ---
class ShopView(ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @ui.button(label="קנה Supporter 🎗️", style=discord.ButtonStyle.secondary, custom_id="shop:supporter", row=0)
    async def buy_supp(self, i: discord.Interaction, b: ui.Button):
        await self.handle_purchase(i, "supporter", 2000, ROLE_SUPPORTER)

    @ui.button(label="קנה VIP 💎", style=discord.ButtonStyle.primary, custom_id="shop:vip", row=1)
    async def buy_vip(self, i: discord.Interaction, b: ui.Button):
        await self.handle_purchase(i, "vip", 5000, ROLE_VIP)

    @ui.button(label="קנה Ticket-Staff 🛠️", style=discord.ButtonStyle.danger, custom_id="shop:staff", row=2)
    async def buy_staff(self, i: discord.Interaction, b: ui.Button):
        await self.handle_purchase(i, "staff", 15000, ROLE_TICKET_STAFF)

    @ui.button(label="בדיקת יתרה 💳", style=discord.ButtonStyle.success, custom_id="shop:bal", row=3)
    async def check_bal(self, i: discord.Interaction, b: ui.Button):
        bal = user_balances.get(i.user.id, 0)
        await i.response.send_message(f"💰 היתרה הנוכחית שלך: `{bal}` מטבעות.", ephemeral=True)

    async def handle_purchase(self, i, name, price, role_id):
        bal = user_balances.get(i.user.id, 0)
        if bal < price:
            return await i.response.send_message(f"❌ חסר לך `{price - bal}` מטבעות לרכישה!", ephemeral=True)
        
        role = i.guild.get_role(role_id)
        if not role:
            return await i.response.send_message("❌ תקלה: הרול לא נמצא בשרת.", ephemeral=True)
        
        if role in i.user.roles:
            return await i.response.send_message("❌ כבר רכשת את הרול הזה!", ephemeral=True)

        user_balances[i.user.id] -= price
        await i.user.add_roles(role)
        await i.response.send_message(f"✅ תתחדש! קיבלת את הרול **{role.name}**!", ephemeral=True)

# --- הגדרות הבוט ---
class CyberBot(commands.Bot):
    def __init__(self):
        intents = discord.Intents.all()
        super().__init__(command_prefix="!", intents=intents)

    async def setup_hook(self):
        self.add_view(ShopView())
        await self.tree.sync()

bot = CyberBot()

@bot.event
async def on_ready():
    print(f"✅ {bot.user.name} Is Online!")

# --- צבירת כסף מהודעות ---
@bot.event
async def on_message(msg):
    if msg.author.bot: return
    # הוספת 10 מטבעות לכל הודעה
    user_balances[msg.author.id] = user_balances.get(msg.author.id, 0) + 10
    await bot.process_commands(msg)

# --- פקודות ניהול (Admin Only) ---

@bot.tree.command(name="add_money", description="הוסף כסף למשתמש")
async def add_money(i: discord.Interaction, member: discord.Member, amount: int):
    if i.user.id != MY_USER_ID: return
    user_balances[member.id] = user_balances.get(member.id, 0) + amount
    await i.response.send_message(f"💵 הבאת `{amount}` מטבעות ל-{member.mention}")

@bot.tree.command(name="remove_money", description="הורד כסף למשתמש")
async def remove_money(i: discord.Interaction, member: discord.Member, amount: int):
    if i.user.id != MY_USER_ID: return
    curr = user_balances.get(member.id, 0)
    user_balances[member.id] = max(0, curr - amount)
    await i.response.send_message(f"📉 הורדת `{amount}` מטבעות ל-{member.mention}")

@bot.tree.command(name="set_money", description="קבע כסף למשתמש")
async def set_money(i: discord.Interaction, member: discord.Member, amount: int):
    if i.user.id != MY_USER_ID: return
    user_balances[member.id] = amount
    await i.response.send_message(f"⚙️ היתרה של {member.mention} עודכנה ל-`{amount}`")

# --- פקודת הקמת החנות המעוצבת ---

@bot.tree.command(name="setup_shop", description="הקמת חנות הרולים המעוצבת")
async def setup_shop(i: discord.Interaction):
    if i.user.id != MY_USER_ID: 
        return await i.response.send_message("🚫 פקודה לאונר בלבד!", ephemeral=True)
    
    emb = discord.Embed(
        title="═══ 💠 CYBER-STORE MARKET 💠 ═══",
        description=(
            "👋 **ברוכים הבאים לחנות הרשמית!**\n"
            "כאן תוכלו להמיר את הפעילות שלכם בשרת לדרגות וסמכויות.\n"
            "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
        ),
        color=0x2b2d31
    )
    
    emb.add_field(
        name="🎗️ | Server-Supporter", 
        value="
http://googleusercontent.com/immersive_entry_chip/0
http://googleusercontent.com/immersive_entry_chip/1
http://googleusercontent.com/immersive_entry_chip/2

### למה הקוד הזה מושלם?
* **העיצוב:** כל פריט קיבל "בלוק" משלו, ורול הטיקט סטאף מופרד עם כותרת יוקרתית.
* **הפרדה:** לכל כפתור יש שורה משלו (`row`), אז זה לא נראה דחוס כמו בתמונה הקודמת.
* **ביטחון:** אין פה טוקן חשוף, זה עובד עם `os.getenv`.
* **תיקון רול:** ה-ID של הטיקט סטאף (1501316672345211041) מעודכן בפנים.

תעלה את זה לגיטהאב, Railway יתקין הכל, ותכתוב `/setup_shop`. זה הולך להיראות פצצה! 🚀🔥
